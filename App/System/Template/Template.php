...
class Template
{
    ...
    protected function compile_layouts($value)
    {
        ...
        $lines[] = preg_replace( $this->matcher('extend'), '$1<?=view$2->with(get_defined_vars())->render();', $lines[0]); // добавляем import в конец,
    ...
    {
        return preg_replace($this->matcher('import'), '$1<?=view$2->render(); ?>', $value);
    }
